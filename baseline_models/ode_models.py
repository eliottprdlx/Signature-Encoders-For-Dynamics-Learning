...
from .encoders import Encoder_z0
...
class GeneralLatentODE(nn.Module):
    """
    GeneralLatentODE is a neural network model for learning latent Ordinary
    Differential Equations (ODEs), supporting both ODE-RNN and signature-based encoders.

    Args:
        dim (int): Input dimensionality.
        obsrv_std (float): Standard deviation of observation noise.
        n_classes (int): Number of classes (for classification).
        model (str): Either 'ode' or 'flow'.
        flow_model (str): Type of flow model (if using 'flow').
        latents (int): Latent space dimensionality.
        hidden_dim (int): Hidden units per layer.
        hidden_layers (int): Number of layers in the ODE function.
        solver (str): Solver for integration (e.g., 'euler').
        z0_encoder (str): Type of encoder_z0: 'ode_rnn' or 'signature'.
        encoder_kwargs (dict): Additional encoder-specific parameters.
    """
    ...
    def __init__(
        ...
        obsrv_std = torch.tensor([obsrv_std], device=DEVICE)
    ...
    def _get_loss(self, dl):
        loss = compute_loss_all_batches(self.model, dl, classify=0, data="", device=DEVICE)
    ...
    def encode(self, dl):
        ...
        for batch in dl:
            ...
            mean, _ = self.model.encoder_z0(obs, batch["observed_tp"], run_backwards=True)
...
def create_LatentODE_model(
    ...
    gru_units=50,
    classify=0,
    encoder_type="ode_rnn",
    encoder_kwargs=None,
):
    classif_per_tp = data == "activity"
    enc_input_dim = int(input_dim) * 2  # input + mask
    ...
    if model == "ode":
        z0_diffeq_solver = SolverWrapper(
            ODEModel(
                rec_dims, odenet, hidden_dims, activation,
                final_activation, solver, solver_step, atol, rtol
            )
        )
        diffeq_solver = SolverWrapper(
            ODEModel(
                latents, odenet, hidden_dims, activation,
                final_activation, solver, solver_step, atol, rtol
            )
        )
    elif model == "flow":
        ...
        z0_diffeq_solver = SolverWrapper(flow(rec_dims, flow_layers, hidden_dims, time_net, time_hidden_dim))
        diffeq_solver = SolverWrapper(flow(latents, flow_layers, hidden_dims, time_net, time_hidden_dim))
    else:
    ...
    if encoder_type == "ode_rnn":
        encoder_kwargs["z0_diffeq_solver"] = z0_diffeq_solver

    encoder_z0 = Encoder_z0(
        input_dim=enc_input_dim,
        encoder_type=encoder_type,
        hidden_units=gru_units,
