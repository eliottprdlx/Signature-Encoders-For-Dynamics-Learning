...
normalization, and model evaluation in neural network experiments.
It includes functions for data splitting, normalization, and loss computation,
as well as classes for variational autoencoders and neural network components.
"""
...
def split_last_dim(data):
  ...
  if len(data.size()) == 3:
    ...
    raise ValueError("Data has to be 2 or 3 dimensional")
  return res
...
def split_data_extrap(data_dict, dataset="", observe_step=1, predict_step=1):
  ...
  split_dict = {
      "observed_data":
          data_dict["data"][:, :n_observed_tp, :][:, ::observe_step, :].clone(),
      "observed_tp":
          data_dict["time_steps"][:, :n_observed_tp][:, ::observe_step].clone(),
      "data_to_predict":
          data_dict["data"][:, n_observed_tp:, :][:, ::predict_step, :].clone(),
      "tp_to_predict":
          data_dict["time_steps"][:, n_observed_tp:][:, ::predict_step].clone(),
  }
...
def compute_loss_all_batches(
    ...
    extrap_factor=1.0# pylint: disable=unused-argument
):
  total = {}
  total["loss"] = 0
  total["likelihood"] = 0
  ...
  for batch_dict in dl:
    ...
    if classify and data != "hopper":
      ...
      classif_predictions = torch.cat(
          (
              classif_predictions,
              results["label_predictions"].reshape(n_traj_samples, -1,
                                                   n_labels),
          ),
          1,
      )
  ...
  if classify:
    if data == "physionet":
      ...
      if torch.sum(all_test_labels) != 0.0:
        print("Number of labeled examples: "
              f"{len(all_test_labels.reshape(-1))}")
        print("Number of examples with mortality 1: "
              f"{torch.sum(all_test_labels == 1.0)}")
        ...
        print(
            "Warning: Couldn't compute AUC -- all examples are from the same "
            "class"
        )
    ...
    if data == "activity":
      ...
      # classif_predictions and all_test_labels are in on-hot-encoding --
...
class VAE_Baseline(nn.Module): # pylint: disable=invalid-name
  """
  Variational Autoencoder (VAE) baseline model for time-series data with
  optional classification and Poisson process support.
  """
  ...
  def compute_all_losses(self, batch_dict, n_traj_samples=1, kl_coef=1.0):
    ...
    results["mse"] = torch.mean(mse_inner).detach()
...
class Encoder_z0_ODE_RNN(nn.Module): # pylint: disable=invalid-name
  """
  Encoder for latent initial state using ODE-RNN with LSTM cells.
  Args:
    latent_dim (int): Dimensionality of the latent space.
    input_dim (int): Dimensionality of the input data.
    z0_diffeq_solver (callable, optional): Solver for the differential equation.
    z0_dim (int, optional): Dimensionality of the initial latent state.
    n_gru_units (int, optional): Number of units in the GRU.
...
class Decoder(nn.Module):
  """
  A neural network decoder that transforms latent space representations
  back to the input space using a linear layer.
  """
...
def gaussian_log_likelihood(mu_2d, data_2d, obsrv_std, indices=None): # pylint: disable=unused-argument
...
def compute_binary_CE_loss(label_predictions, mortality_label): # pylint: disable=invalid-name
  ...
  if torch.sum(mortality_label == 0.0) == 0 or torch.sum(
    ...
    print(
        "Warning: all examples in a batch belong to the same class -- "
  ...
  # For each trajectory, we get n_traj_samples samples from z0 -- compute
  # loss on all of them
...
def compute_multiclass_CE_loss(label_predictions, true_label, mask): # pylint: disable=invalid-name
  ...
  # For each trajectory, we get n_traj_samples samples from z0 --
  # compute loss on all of them
  ...
  # repeat the mask for each label to mark that the label for this time point
  # is present
...
def compute_masked_likelihood(mu, data, mask, likelihood_func):
  # Compute the likelihood per patient and per attribute so that we don't
  # priorize patients with more measurements
...
def masked_gaussian_log_density(mu, data, obsrv_std, mask=None):
  ...
  if mask is None:
    ...
    # Compute the likelihood per patient so that we don't priorize patients
...
def mse(mu, data, indices=None): # pylint: disable=unused-argument
  n_data_points = mu.size()[-1]

  if n_data_points > 0:
    return nn.MSELoss()(mu, data)
  else:
    return torch.zeros([1]).to(data).squeeze()
