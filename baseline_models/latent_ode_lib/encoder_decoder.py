...
class Encoder_z0_signature(nn.Module):
  def __init__(self,
      ...
      self.augment = signatory.Augment(in_channels=dimension_in,
                                        layer_sizes=(hidden_units // 2, hidden_units // 2, n_features),
  ...
  def forward(self, observed_data, observed_tp, **kwargs):
    ...
    if self.encode_obs_time:
      trajs_to_encode = torch.cat(
          ...
          dim=2)
    x = self.augment(trajs_to_encode)
    if x.size(1) <= 1:
...
class Encoder_z0_logsignature_rnn(nn.Module):
  def __init__(self,
              latent_dim,
              dimension_in,
              hidden_units=32,
      ...
      self.augment = signatory.Augment(in_channels=dimension_in,
                                         ...
                                         include_time=True)
      self.sig_channels = signatory.logsignature_channels(in_channels=dimension_in+1,
                                                  depth=depth)
      self.signature = signatory.LogSignature(depth=depth, stream=False)
      self.gru = nn.GRU(2*self.sig_channels, hidden_units, 2, batch_first=True)
      self.decay_factor = decay_factor
      self.decay_linear = decay_linear
      self.linear_out = torch.nn.Linear(hidden_units, 2*z0_dim)
      nn.init.xavier_uniform_(self.linear_out.weight)
      self.latent_dim = latent_dim
      self.z0_dim = z0_dim
    ...
    if self.encode_obs_time:
      trajs_to_encode = torch.cat(
          (observed_data, observed_tp.view(1, -1, 1).repeat(
              observed_data.shape[0], 1, 1)),
          dim=2)
    x = self.augment(trajs_to_encode)  # x: (B, L, F)
    L = x.shape[1]
    L_temp = L

    segments = []
    while L - L_temp < L:
        segments.append((L - L_temp, L))
        L_temp = int(L_temp * self.decay_factor) - self.decay_linear

    T = len(segments)
    B = x.shape[0]
    D = self.sig_channels  # infer signature dim

    # Pre-allocate sigs tensor
    sigs = torch.empty(B, T, 2*D, device=x.device, dtype=x.dtype)

    for t, (start, end) in enumerate(segments):
        sig1 = self.signature(x[:, start:end, :], basepoint=True)
        sig2 = self.signature(x[:, start:, :], basepoint=True)
        total_sig = torch.cat((sig1, sig2), dim=1)  # (B, 2*D)
        sigs[:, t, :] = total_sig

    out, _ = self.gru(sigs)
    z = self.linear_out(out[:, -1, :])
    mean_z0, std_z0 = z[:, :self.z0_dim], z[:, self.z0_dim:]
    mean_z0, std_z0 = mean_z0.unsqueeze(0), std_z0.unsqueeze(0)
    std_z0 = F.softplus(std_z0)
    return mean_z0, std_z0
